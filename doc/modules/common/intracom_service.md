...
using a shared medium such as a database or an MQTT broker. This setup ensures that when one module needs to perform an 
update or trigger a resynchronization, other modules can be notified accordingly and take the appropriate action.
...
```properties
# select the intracom medium, currently, only mongo is supported
