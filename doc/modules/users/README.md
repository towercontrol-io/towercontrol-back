...
- self registration, enabled by `users.registration.self` property:
    ...
    - Following the confirmation email, the user sets the other information (passwords...), to create the user structure and assign
      the `ROLE_PENDING_USER` role.
    ...
    - The user can then be validated by an administrator to assign the `ROLE_REGISTERED_USER` role or automatically moved to that status
      depending on the configuration boolean parameter `user.pending.autovalidation`
